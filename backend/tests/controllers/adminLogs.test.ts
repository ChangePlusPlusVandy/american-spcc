...
describe("adminLogsController", () => {
  ...
  describe("createAdminLog", () => {
    it("should create an admin log successfully", async () => {
      ...
      const req = mockReq({ body: { admin_id: "admin123", action: "DELETE_USER", details: "Removed user A" } });
      const res = mockRes();
    ...
    it("should return 404 if admin not found", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ error: "Admin not found or invalid role" }));
    });
    ...
    it("should handle Prisma errors gracefully", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ error: "Failed to create admin log" }));
    });
  });
  ...
  describe("getAllAdminLogs", () => {
    it("should fetch all logs successfully", async () => {
      const mockLogs = [
        { id: "1", admin_fk: "admin123", action: "TEST", created_at: new Date() },
      ];
    ...
    it("should handle Prisma errors gracefully", async () => {
      ...
      expect(res.status).toHaveBeenCalledWith(500);
      expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ error: "Failed to fetch admin logs" }));
    });
  });
});
